...
ada = Adadelta(lr=0.005, rho=0.95, epsilon=1e-08)
...
def main(pathpic):
	...
	ListOfFaces,CoordinatesList = loadData(pathpic)

	FinalResult = []
	start_time = time.time()

	for face, coors in zip(ListOfFaces, CoordinatesList):
		result = predict(face)
		dict = {'FaceRectangle':{ 'left':(coors[0]),'top':(coors[1]),'right':(coors[2]),'bottom':(coors[3])},
		'Scores':{'anger':str(result[0]),'disgust':str(result[1]),'fear':str(result[2]),'happy':str(result[3]),'sad':str(result[4]),'surprise':str(result[5]),'neutral':str(result[6])}}
		FinalResult.append(dict)

	Toreturn = {'data':json.dumps(FinalResult,indent=3, sort_keys=True),'url':pathpic}
	print("--- %s seconds ---" % (time.time() - start_time))
